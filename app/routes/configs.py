...
def get_shift_type():
    try:
        ...
        }, {'_id': 0,'CONFIG_TYPE': 0, 'DEPARTMENT': 0,
            'COMPANY': 0 }) 
        return jsonify({
            'success': True,
            'data': existing_config['SHIFT_TYPE'] if existing_config else shift_types
        }), 200
...
def update_shift_type():
    try:
        ...
        if not data or 'DEPARTMENT' not in data or 'COMPANY' not in data or 'SHIFT_TYPE' not in data:
            return jsonify({
                'success': False,
        ...
        if existing_config:
            ...
            existing_types_dict = {shift['type']: shift for shift in existing_shift_types}
...
def get_role_groups():
    try:
        ...
        return jsonify({
            'success': False,
            'message': 'Error fetching role groups',
            'error': str(e)
        }), 500
...
def update_role_groups():
    try:
        ...
        if not data or 'DEPARTMENT' not in data or 'COMPANY' not in data or 'GROUPS' not in data:
            return jsonify({
                'success': False,
        ...
        if existing_config:
            ...
            for new_group in new_groups:
                ...
                if group_label in existing_groups_dict:
                    ...
                    existing_groups_dict[group_label]['role_list'] = new_group.get('role_list', [])
                else:
                    # Add new group
        ...
        return jsonify({
            'success': False,
            'message': 'Error updating role groups',
            'error': str(e)
        }), 500